...
`
  ...
  const fullSql = [
    '-- ========================================',
    `-- 级联资源配置 - ${escapeSqlValue(form.platformType)}(${escapeSqlValue(form.platformCode)})`,
    `-- 平台编码：${escapeSqlValue(form.platformType)}`,
    `-- 平台英文缩写：${escapeSqlValue(form.platformCode)}`,
    `-- 平台名称：${escapeSqlValue(form.platformName)}`,
  ...
  "data": {
    "records": [...]
      ...
      <div v-if="levels.length > 0" class="mt-4 flex gap-2">
        ...
        <el-button v-if="currentLevel?.fields.length > 0" @click="handlePreview">
          👁️ 预览配置
        </el-button>

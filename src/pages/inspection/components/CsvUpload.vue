...
function handleFileCommand(index: number, command: string) {
...
function removeFieldFromFile(fileIndex: number, field: string) {
  const file = uploadedFiles.value[fileIndex]
  file.columns = file.columns.filter(f => f !== field)
}

function handleAliasFocus(index: number) {
  ...
  <div class="csv-upload">
    ...
    <div v-if="uploadedFiles.length > 0" class="file-list">
      ...
      <div v-for="(file, index) in uploadedFiles" :key="file.id" class="file-item">
        ...
        <div class="file-actions">
          ...
          <el-dropdown @command="(cmd) => handleFileCommand(index, cmd)">
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item command="editAlias">
                  重命名别名
                </el-dropdown-item>
                <el-dropdown-item command="viewFields">
                  查看字段
                </el-dropdown-item>
      ...
      <div class="file-actions-bar">
        <el-button type="primary" :disabled="uploadedFiles.length === 0" @click="handleNextStep">
          下一步：配置规则逻辑
        </el-button>
      </div>
    </div>
